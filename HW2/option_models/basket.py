...
def basket_price_mc_cv(
    ...
    '''
    price3 = basket_price_norm_analytic(strike, spot, vol, weights, texp, cor_m, intr, divr, cp_sign)
...
def basket_price_mc(
    ...
    if( bsm ) :
        ...
        wt = np.sqrt(texp)*chol_m@znorm_m

        prices = forward[:,None]*np.exp(-vol**2/2*texp).reshape(4,1)*np.exp(wt)
...
def basket_price_norm_analytic(
    ...
    price_weighted = weights.reshape(1,4)@forward
    
    vol_bsk = weights.reshape(1,4)@cov_m@weights

    vol_bsk = np.sqrt(vol_bsk)
    
    price = normal_formula(strike,price_weighted , vol_bsk, texp, intr, 0, cp_sign)
    return price
